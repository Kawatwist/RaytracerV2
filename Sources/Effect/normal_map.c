...
/*   Updated: 2020/01/13 18:22:10 by lomasse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
**                uv.x += (uv.x < 0 ? (info & 0xFFFF) - 1 :
**					-(info & 0xFFFF) - 1);
**            while (uv.y < 0.0 || uv.y >= (data.normal[info >> 16])->h)
**                uv.y += (uv.y < 0 ? (data.normal[info >> 16])->h - 1 :
**					-(data.normal[info >> 16])->h) - 1;
**            index = (uv.x + (uv.y * (info & 0xFFFF))) * 4;
**            normal2.x = (((((unsigned char)(data.normal[info >> 16]
**				->data[index + 1]) / 0x7F) - 0.5) *
**				((t_base*)obj)->effect.normal / 255.0));
**            normal2.y = (((((unsigned char)(data.normal[info >> 16]
**				->data[index + 2]) / 0x7F) - 0.5) *
**				((t_base*)obj)->effect.normal / 255.0));
**            normal2.z = 2;
**            return (normal2);
**        }
