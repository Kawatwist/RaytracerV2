...
/*   Updated: 2020/07/21 19:30:45 by lomasse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int		send_aa_vr(t_thread *data, int x, int y, int curr)
{
	...
	if (curr % data->x < data->x / 2.0)
		color = send_ray(data, setup_ray(data,
					...
					y + (((rand() % 100) - 50) / 200.0), 0), data->bounce);
	else
		color = send_ray(data, setup_ray(data,
					((x - (data->x / 2.0)) * 2.0) + (((rand() % 100) - 50) / 200.0),
					y + (((rand() % 100) - 50) / 200.0), 1), data->bounce);
	return (color);
}
	...
	while (++i < aa)
	{
		if (data->obj.camera[data->obj.index[0]].mode != 2)
		{
			...
			if (data->obj.camera[data->obj.index[0]].mode == 1)
				color[i] = (send_ray(data, setup_ray(data,
				*x + (((rand() % 100) - 50) / 200.0),
				*y + (((rand() % 100) - 50) / 200.0), 1), data->bounce) & 0xFFFF)
					| (color[i] & 0xFF0000);
		}
		else
