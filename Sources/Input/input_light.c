...
/*   Updated: 2020/07/29 13:49:48 by lomasse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (key_hold(data, SDL_SCANCODE_KP_1) && !ctrl)
		(*(t_light **)light)->distance += 0.1;
	else if (key_hold(data, SDL_SCANCODE_KP_1) && ctrl &&
...
{
	if (key_hold(data, SDL_SCANCODE_KP_2) && !ctrl &&
	(*(t_light **)light)->intensity < 1.0)
		(*(t_light **)light)->intensity += 0.1;
	else if (key_hold(data, SDL_SCANCODE_KP_2) && ctrl &&
...
void	input_color_light(t_data *data, void **light, char ctrl)
{
	...
	((*(t_light **)light)->color & 0xFF0000) >> 16 < 255)
		((char *)&((*(t_light **)light)->color))[2] += 1;
	else if (key_hold(data, SDL_SCANCODE_KP_7) && ctrl &&
	((*(t_light **)light)->color & 0xFF0000) >> 16 > 0)
		((char *)&((*(t_light **)light)->color))[2] -= 1;
	if (key_hold(data, SDL_SCANCODE_KP_8) && !ctrl &&
	((*(t_light **)light)->color & 0xFF00) >> 8 < 255)
		((char *)&((*(t_light **)light)->color))[1] += 1;
	else if (key_hold(data, SDL_SCANCODE_KP_8) && ctrl &&
	((*(t_light **)light)->color & 0xFF00) >> 8 > 0)
		((char *)&((*(t_light **)light)->color))[1] -= 1;
	if (key_hold(data, SDL_SCANCODE_KP_9) && !ctrl &&
	((*(t_light **)light)->color & 0xFF) < 255)
		((char *)&((*(t_light **)light)->color))[0] += 1;
	else if (key_hold(data, SDL_SCANCODE_KP_9) && ctrl &&
	((*(t_light **)light)->color & 0xFF) > 0)
		((char *)&((*(t_light **)light)->color))[0] -= 1;
}
